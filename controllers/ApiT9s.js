...
GetTransfer = async (req, res) => {
    var { Username, Password, Limit, Type } = req.body
    const user = await User.findOne({ Username: Username })
    if (user) {
        const isValidPass = Helper.validPassword(Password, user.Password)
        if (!isValidPass) {
...
Transfer = async (req, res) => {
    var { Username, Password, PasswordLever2, UsernameTo, AmountTransFer, Comment } = req.body
    const user = await User.findOne({ Username: Username })
    const userTo = await User.findOne({ Username: UsernameTo })
    if (!userTo) {
